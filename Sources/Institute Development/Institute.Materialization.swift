...
extension Institute {
  /// One local materialisation registered on this machine: an identity, the
  /// physical root it currently resolves to, and how Institute may treat
  /// that root's filesystem content.
  ///
  ...
  /// ownership, and persistence: no branch, revision, package identity, or
  /// repository membership is recorded here, and none ever should be.
  public struct Materialization: Swift.Equatable, Swift.Sendable {
    /// The registry key. Never a filesystem path.
    public let id: ID

    /// Where ``id`` currently resolves on disk. Not identity: this can
    /// change underneath a stable ``id`` (see ``relocated(to:)``), and two
    /// distinct ids are never permitted to resolve to the same physical
    /// directory (enforced by ``Registry``, not by this type).
...
extension Institute.Materialization {
  /// This materialisation with ``locator`` replaced and everything else,
...
extension Institute.Materialization: JSON.Serializable {
  ...
  public static func deserialize(_ json: JSON) throws(JSON.Error) -> Self {
    ...
    guard actual == expected else {
      throw .typeMismatch(
        expected: "materialization keys id, locator, and ownership",
    ...
    return try Self(
      id: Institute.Materialization.ID(json: id),
      locator: File.Directory(path),
      ownership: Institute.Materialization.Ownership(json: ownership)
    )
  }
}
