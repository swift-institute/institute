...
extension Institute.Composition {
    ...
    private func structuralCheck(
        ...
        do throws(File.System.Write.Atomic.Error) {
            ...
            throw .composition(
                "structural check: cannot stage the manifest for evaluation: \(error)"
            )
        }
    ...
    private func report(composed record: Record, manifest: File) {
        ...
        print(
            "      Run `institute restore --consumer \(record.consumer) --dependency \(record.dependency)` before pushing."
        )
        print(
            "      Build through the coordinator; then `institute verify` to confirm the effective source."
        )
    }
    ...
    private func report(restored record: Record, manifest: File) {
        ...
        print("")
        print(
            "  Structural check (resolve-free): the restored manifest evaluates, \(record.dependency)"
        )
        ...
        print(
            "  \(record.dependency) resolves from its canonical remote — the reproducibility check"
        )
    ...
    private func report(
        ...
        print(
            "  ledger: \(composed ? "composed (local development source active)" : "no active composition")"
        )
        switch state {
        ...
        if composed != isLocal {
            ...
            print(
                "      current ledger — re-resolve through your build coordinator and verify again."
            )
        }
    }
}
