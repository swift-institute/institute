...
extension Institute.Context.Packet.Remote {
  ...
  private static func parent(of key: Institute.Context.Packet.Key) async -> (
    value: Swift.String?, diagnostics: [Swift.String]
  ) {
  ...
  private static func children(of key: Institute.Context.Packet.Key) async -> (
    value: [Swift.String], diagnostics: [Swift.String]
  ) {
  ...
  private static func comment(at url: Swift.String) async
    -> Institute.Context.Packet.Fetch<Institute.Context.Packet.Record.Comment>
  {
  ...
  private static func strings(_ document: JSON, key: Swift.String) throws(JSON.Error) -> [Swift
    .String]
  {
    guard let values = document.array else {
  ...
  private static func commentIssue(_ endpoint: JSON) throws(JSON.Error)
    -> Institute.Context.Packet.Key
  {
    ...
    guard let key = Institute.Context.Packet.Key(argument: "\(parts[0])/\(parts[1])#\(parts[3])")
    else {
      throw .typeMismatch(expected: "GitHub Issue URL", got: url)
    }
    return key
  ...
  private static func json(_ endpoint: Swift.String, absent: Swift.Bool = false) async
