...
extension Institute.Context.Packet.Remote {
    ...
    private static func parent(
        of key: Institute.Context.Packet.Key
    ) async -> (
        value: Swift.String?, diagnostics: [Swift.String]
    ) {
        switch await json(
            "repos/\(key.repository.identity)/issues/\(key.number)/parent",
            absent: true
        )
        {
    ...
    private static func children(
        ...
        case .available(let document):
            ...
            do {
                ...
                return ([], ["\(key.identity): malformed native sub-issues response: \(error)"])
            }
        ...
        -> Institute.Context.Packet.Fetch<Institute.Context.Packet.Record.Comment>
    {
        ...
        if let start = url.range(of: "/repos/")?.lowerBound {
            ...
            let pieces = url.dropFirst("https://github.com/".count).split(
                separator: "#",
                maxSplits: 1
            )
        ...
        do {
            return .available(
                .init(
                    ...
                    body: {
                        do throws(JSON.Error) {
                            return try Swift.String.deserialize(document["body"])
                        } catch {
                            return ""
                        }
                    }()
    ...
    private static func commentIssue(
        _ endpoint: JSON
    ) throws(JSON.Error)
        -> Institute.Context.Packet.Key
    {
        ...
        let parts = url.dropFirst(marker.count).split(
            separator: "/",
        ...
        guard
            let key = Institute.Context.Packet.Key(argument: "\(parts[0])/\(parts[1])#\(parts[3])")
        else {
            throw .typeMismatch(expected: "GitHub Issue URL", got: url)
        }
        return key
