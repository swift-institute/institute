...
extension Institute.Context {
    ...
    /// The pre-TX-APP1Z marker, accepted alongside ``marker`` for one migration
    /// window so a projection installed before the rename is still recognized
    /// as managed instead of refused as unmanaged.
    ///
    /// REMOVAL GATE — 2026-10-01 (post-launch): every projection in the wild
    /// will have been replaced with the current marker by then (replacement is
    /// self-healing — see `conflicts(documents:links:)`), so this compatibility
    /// prefix can be deleted along with the sites that check it and the test
    /// that pins its presence.
    private static let legacyMarker =
        "<!-- Generated by `workspace context install`; edit the canonical template in Workspace/Context. -->"

    /// Whether `document` carries a recognized managed-projection marker,
    /// current or legacy. A replaced projection always writes the current
    /// marker (self-healing), so acceptance of the legacy prefix only ever
    /// widens what an existing installation is recognized as, never what a
    /// fresh install produces.
    private static func isManaged(_ document: Swift.String) -> Bool {
        document.hasPrefix(marker) || document.hasPrefix(legacyMarker)
    }
...
extension Institute.Context {
    private func conflicts(
        ...
        for document in documents {
            ...
            guard document.marker != nil, Self.isManaged(actual) else {
                findings.append(
        ...
        for link in links {
            ...
            if link.path == entry[file: "CLAUDE.md"].path,
                info.type == .regular,
                try Self.isManaged(read(File(link.path)))
            {
                continue
            }
