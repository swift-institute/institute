...
extension Institute.Hierarchy {
  /// An immutable, validated registry key for one hierarchy.
  ///
  ...
  /// Hierarchy/Registry`` decides whether two ids may coexist.
  public struct ID: Swift.Equatable, Swift.Hashable, Swift.Sendable, Swift.CustomStringConvertible
  {
    ...
    public init(_ value: Swift.String) throws(Institute.Error) {
      guard !value.isEmpty else {
        throw .configuration("hierarchy id must not be empty")
      }
      guard value != ".", value != ".." else {
        throw .configuration("hierarchy id must not be a traversal component")
      }
      do throws(File.Path.Component.Error) {
        _ = try File.Path.Component(value)
      } catch {
        throw .configuration(
          "hierarchy id \(value) is not a single valid path component: \(error)"
        )
      }
      self.value = value
    }
...
extension Institute.Hierarchy.ID: JSON.Serializable {
  ...
  public static func deserialize(_ json: JSON) throws(JSON.Error) -> Self {
    ...
    do throws(Institute.Error) {
      ...
      throw .typeMismatch(expected: "a valid hierarchy id", got: "\(raw): \(error)")
    }
  }
}
