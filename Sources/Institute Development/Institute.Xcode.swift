...
extension Institute.Xcode {
  ...
  public static func document(
    ...
    for member in specification.members {
      ...
      references.append(.file(location))
    }
  ...
  public static func write(
    ...
    let workspaceContents = try render(specification)
    let specificationContents = specification.jsonString(sortKeys: true) + "\n"
    do throws(File.System.Write.Atomic.Error) {
      try path(at: root).write.atomic(workspaceContents)
      try specificationPath(at: root).write.atomic(specificationContents)
    } catch {
