    ...
    public func validated() throws(Institute.Error) -> Self {
        ...
        for repository in repositories {
            ...
            guard names.insert(repository.name).inserted else {
                throw .configuration(
                    "Institute.json contains duplicate repository name \(repository.name)"
                )
            }
            ...
            if let root = Institute.Layer.allCases.first(where: {
                $0.organization == repository.organization
            }),
