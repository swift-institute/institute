...
extension Institute.Doctor {
    ...
    func resolutionCurrency(
        ...
        var comparisons:
            [Swift.String: Swift.Result<
                (behind: Swift.Int, pinnedAt: Swift.String), Institute.Error
            >] =
                [:]
    ...
    private static func diagnostic(
        ...
        case .success, nil:
            "its comparison was never requested — the gather missed its own population"
        }
    }
    ...
    private func compare(
        ...
        do throws(JSON.Error) {
            ...
            throw .configuration(
                "compare response for \(key.identity) carries no ahead_by: \(error)"
            )
        }
