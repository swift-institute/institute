...
extension Institute.Doctor {
        ...
        case .discovered:
            [
                .init(
                    severity: .error,
                    message:
                        "\(repository.name): discovered on GitHub but missing from Institute.json"
                )
            ]
    ...
    func currency(_ discovery: Institute.Inventory.Discovery) -> Outcome {
        ...
        for repository in configuration.repositories {
            committedByCoordinate[
                .init(organization: repository.organization, name: repository.name)
            ] =
                repository
        }
