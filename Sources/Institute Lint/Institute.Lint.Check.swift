...
extension Institute.Lint.Check {
  ...
  static func resolve(_ tool: Swift.String) -> File? {
    ...
    for directory in path.split(separator: separator, omittingEmptySubsequences: true) {
      guard let component = try? File.Path.Component(Swift.String(tool)) else { return nil }
      guard let directoryPath = try? File.Path(Swift.String(directory)) else { continue }
...
extension Institute.Lint.Check {
  ...
  func swiftFormat(
    ...
    let arguments =
      ["lint", "--strict", "--ignore-unparsable-files", "--configuration", configuration.description]
    ...
    case .exited(let code):
      let diagnostics = Swift.String(decoding: output.stderr ?? [], as: Swift.UTF8.self)
  ...
  func swiftLint(
    ...
    switch output.status {
    case .exited(let code):
      guard standardOutput.contains("Done linting!") || standardOutput.contains("warning")
...
extension Institute.Lint.Check {
  ...
  private func finish(
    ...
    // swift-linter: unchanged, byte-for-byte what `package lint` runs.
    do throws(Institute.Error) {
