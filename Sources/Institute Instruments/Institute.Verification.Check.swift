...
extension Institute.Verification.Check {
    ...
    public static func diagnostics(for receipt: Institute.Verification.Receipt) -> [Swift.String] {
        ...
        if receipt.version != 1 {
            diagnostics.append(
                "unsupported schema version \(receipt.version); this validator knows 1"
            )
        }

        if receipt.requestedOperations.isEmpty {
            diagnostics.append(
                "requestedOperations is empty; a receipt with nothing requested "
        ...
        if !receipt.operations.contains(where: { $0.outcome.isExecuted }) {
            diagnostics.append(
                "no operation reached a real outcome; a receipt attesting nothing was "
        ...
        case .verified:
            ...
            for gate in receipt.requiredGates where !gate.satisfied {
                diagnostics.append(
                    "verdict is verified but required gate \(gate.name) is unsatisfied"
                )
            }
