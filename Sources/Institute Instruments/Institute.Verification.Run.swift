...
extension Institute.Verification.Run {
    ...
    struct Tools: Sendable {
        ...
        init(
            ...
            build:
                @escaping @Sendable (Swift.String, Swift.Bool, Swift.Int?, [Swift.String]) ->
            ...
            nestedTests:
                @escaping @Sendable (Swift.String, Swift.Bool, Swift.Int?, [Swift.String]) ->
                [Institute.Verification.Operation.Result] = Institute.Verification.Run
                .realNestedTests,
...
extension Institute.Verification.Run {
    /// The caller-supplied facts one ``run(_:action:invocation:)`` call
    /// needs beyond the operation and the `Build.Action` itself — grouped
    /// so that call bundles a single value rather than six positional
    /// facts.
    struct Invocation: Sendable {
        ...
        let started: Swift.String
        let now: @Sendable () -> Swift.String
    }

    /// Runs one `Build.Action` at `invocation.path` and folds the
    /// coordinator's result into an ``Operation/Result``. Shared by
    /// ``realBuild``, ``realTest``, and each nested test package
    ...
    static func run(
        ...
        return .init(
            operation: operation,
            subpath: invocation.subpath,
            arguments: invocation.arguments,
            startedAt: invocation.started,
            endedAt: invocation.now(),
            durationSeconds: Self.seconds(clock.now - clockStart),
            exitCode: exitCode,
            provenance: invocation.fresh ? .fresh : .cached,
            outcome: outcome,
...
extension Institute.Verification.Run {
    ...
    public func run() throws(Institute.Verification.Error) -> Institute.Verification.Receipt {
        ...
        for result in results {
            ...
            {
                throw .unsafeContent(
                    "operation \(result.operation.rawValue) compile evidence \(reason)"
                )
            }
