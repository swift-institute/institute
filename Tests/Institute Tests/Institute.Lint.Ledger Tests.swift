...
extension Institute.Lint.Ledger.Test.Unit {
    ...
    func `ledger is a complete census with exact residual and ownership state`() throws {
        ...
        let report = try Institute.Lint.Ledger.Report(
            repositories: repositories,
            report: Institute.Lint.Ledger.Test.sweep(repositories: repositories),
        ...
        #expect(
            report.packages[0].verification?.revision == Swift.String(repeating: "a", count: 40)
        )
    ...
    func `complete measured evidence distinguishes errors from compliance`() throws {
        ...
        let repositories = Array(Institute.Lint.Ledger.Test.repositories.prefix(2))
        let noncompliant = try Institute.Lint.Ledger.Report(
            repositories: repositories,
            report: Institute.Lint.Ledger.Test.sweep(repositories: repositories),
            dispositions: [disposition],
            verifications: []
        ...
        let compliant = try Institute.Lint.Ledger.Report(
            repositories: [compliantRepository],
            report: Institute.Lint.Ledger.Test.sweep(
                repositories: [compliantRepository]
            ),
            dispositions: [disposition],
            verifications: []
    ...
    func `human and JSON encodings are deterministic across input order`() throws {
        ...
        let forward = try Institute.Lint.Ledger.Report(
            repositories: repositories,
            report: Institute.Lint.Ledger.Test.sweep(repositories: repositories),
            dispositions: [disposition],
            verifications: []
        )
        let reverseSweep = Institute.Lint.Ledger.Test.sweep(repositories: repositories)
        let reverse = try Institute.Lint.Ledger.Report(
...
extension Institute.Lint.Ledger.Test.`Edge Case` {
    @Test
        ...
        let report = try Institute.Lint.Ledger.Report(
            ...
            report: .init(
                ...
                unmaterialized: repositories.compactMap {
                    Institute.Repository.Key(repository: $0)?.identity
                },
                considered: 0,
                measurements: []
            ),
    ...
    func `missing structured evidence is an explicit blocked row`() throws {
        let repository = Institute.Lint.Ledger.Test.repositories[0]
        let key = try #require(Institute.Repository.Key(repository: repository))
    ...
    func `typed prerequisite state is independent of arbitrary reason prose`() throws {
        let typed = try Institute.Lint.Ledger.Test.unmeasured(
            reason: "wording can change without changing machine state",
            prerequisite: .sarif
        )
        let prose = try Institute.Lint.Ledger.Test.unmeasured(
            reason:
                "structured findings and \(Institute.Lint.Ledger.Report.prerequisite) appear only "
        ...
        #expect(
            typed.description.contains(
                "prerequisite: sarif · \(Institute.Lint.Ledger.Report.prerequisite)"
            )
        )
...
extension Institute.Lint.Ledger.Test {
    ...
    fileprivate static func sweep(
        ...
        let measurements = materialized.map { repository in
            ...
            return Institute.Lint.Measurement(
                repository: key,
    ...
    ) throws -> Institute.Lint.Ledger.Report {
