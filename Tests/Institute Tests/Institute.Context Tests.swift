...
extension Institute.Context.Test {
    ...
    /// The pre-TX-APP1Z marker, accepted for one migration window — see the
    /// removal gate on `Institute.Context.legacyMarker` (2026-10-01).
    fileprivate static let legacyMarker =
    ...
    private static func fixture<Result>(
        ...
        let checkout = institute[directory: "Institute"]
        ...
        return try body(
            Institute.Context(
                root: Institute.Root(checkout: checkout),
...
extension Institute.Context.Test.Unit {
    ...
    func `install materializes documents and every canonical skill source`() throws {
        try Institute.Context.Test.fixture { context in
            ...
            let skills = Institute.Context.Test.projections(context)
    ...
    func `install refreshes its generated document`() throws {
        try Institute.Context.Test.fixture { context in
            ...
            try context.root.checkout[directory: "Context"][file: "AGENTS.md"]
                .write.atomic("\(Institute.Context.Test.marker)\n# Revised fixture\n")
    ...
    @Test
    func `install replaces a document carrying the legacy pre-rename marker`() throws {
        try Institute.Context.Test.fixture { context in
            try context.entry[file: "AGENTS.md"].write.atomic(
                "\(Institute.Context.Test.legacyMarker)\n# Stale fixture\n"
            )
...
extension Institute.Context.Test.`Edge Case` {
    @Test
    ...
    /// The legacy prefix is a bounded, dated compatibility exposure (removal
    /// gate 2026-10-01), not a general relaxation of the refusal check — a
    /// document carrying neither the current nor the legacy marker must still
    /// be refused.
    @Test
    func `install still refuses a document with an unrecognized marker`() throws {
        try Institute.Context.Test.fixture { context in
            try context.entry[file: "AGENTS.md"].write.atomic(
    ...
    func `install projects the canonical roots this account carries and skips the rest`() throws {
        try Institute.Context.Test.fixture(roots: ["public-skill"]) { context in
            try context.install()
    ...
    func `install leaves a projection into a root it does not own alone`() throws {
        try Institute.Context.Test.fixture(roots: ["public-skill"]) { context in
            ...
            // root — a personal dotfiles checkout, say. Institute does not know
...
extension Institute.Context.Test.`Edge Case` {
    ...
    func `install with no canonical skill root fails and names every root it looked for`() throws {
        try Institute.Context.Test.fixture(roots: []) { context in
            #expect(throws: Institute.Error.self) {
                try context.install()
            }
    ...
    func `install reports the count and the roots it projected from`() throws {
        try Institute.Context.Test.fixture(roots: ["public-skill", "rule-skill"]) { context in
