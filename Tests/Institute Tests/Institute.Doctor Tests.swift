...
extension Institute.Doctor.Test.Unit {
    ...
    @Test
    func
        `an over-firing evaluation is caught by the known-negative control and aborts at unmeasured`()
    {
...
extension Institute.Doctor.Test.Unit {
    ...
    @Test
    func
        `a run containing unmeasured is textually distinct from ok and never described as passing`()
    {
        let report = Institute.Doctor.Report(outcomes: [
            Self.outcome("a", .ok(population: 2)),
...
extension Institute.Doctor.Test.Integration {
    ...
    @Test
    func `a canonical sibling checkout is the only materialized state accepted by doctor`()
        async throws
    {
    ...
    @Test
    func `a legacy in checkout repository is not counted as canonical materialization`()
        async throws
    {
        let repository = Institute.Repository(
            name: "swift-example",
    ...
    @Test
    func
        `both canonical and legacy repositories are a doctor conflict rather than an ambiguous success`()
        async throws
    {
        let repository = Institute.Repository(
            name: "swift-example",
    ...
    @Test
    func
        `an empty Packages population against a non-empty inventory reports unmeasured and exits 2`()
        async throws
    {
            ...
            origin: .overridden(
                ...
                removed: [
                    .init(
                        ...
                        name: .init("swift-dimension-primitives")
                    )
                ]
            )
        )
