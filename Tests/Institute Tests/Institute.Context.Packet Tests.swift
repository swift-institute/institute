...
extension Institute.Context.Packet.Test {
    ...
    func `key accepts a canonical Issue coordinate only`() {
        #expect(
            Institute.Context.Packet.Key(argument: "swift-institute/institute-application#100")?
                .identity
                == "swift-institute/institute-application#100"
        )
        #expect(
            Institute.Context.Packet.Key(argument: "swift-institute/institute-application#0") == nil
        )
        #expect(
            Institute.Context.Packet.Key(argument: "swift-institute/institute-application") == nil
        )
        #expect(
            Institute.Context.Packet.Key(argument: "swift-institute/institute-application#one")
                == nil
        )
    }
    ...
    func `report has deterministic bounded JSON and states its continuation`() {
        let report = Institute.Context.Packet.Report(
            record: .init(
                key: Institute.Context.Packet.Key(
                    argument: "swift-institute/institute-application#100"
                )!,
    ...
    func `incomplete evidence exits two rather than looking clean`() {
        let report = Institute.Context.Packet.Report(
            record: nil,
            diagnostics: ["GitHub unavailable"],
            maxBytes: 24_000
        )
    ...
    func `human packet truncation preserves a non ASCII scalar boundary`() {
        ...
        let maxBytes = 501
        let rendered = Institute.Context.Packet.Report(
            record: record,
            diagnostics: [],
            maxBytes: maxBytes
        ).render(.human)
    ...
    func `measured comment mismatch exits one`() {
        let record = Institute.Context.Packet.Record(
            ...
            children: [],
            comments: [],
            divergences: ["included comment belongs to another Issue"],
            diagnostics: []
        )
        #expect(
            Institute.Context.Packet.Report(record: record, diagnostics: [], maxBytes: 24_000)
                .status == 1
        )
    }
}
