...
extension Institute.Doctor.Test.Unit {
    ...
    @Test
    func
        `dirty entries, untracked entries, and a feature branch are warnings that never raise the exit`()
    {
        let outcome = Institute.Doctor.census.run(
            population: [
...
extension Institute.Doctor.Test.Unit {
    @Test
    func
        `a stale pin warns, naming the dependency, the pinned revision, the tip, and re-resolution`()
    {
