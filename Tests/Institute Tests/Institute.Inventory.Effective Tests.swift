...
extension Institute.Inventory.Test.Unit {
    ...
    func `Effective combines the committed public roster with a live private discovery`() throws {
        let publicConfiguration = Institute.Configuration(
            version: 1,
    ...
    func `A private repository name colliding with a different public owner fails closed`() {
        ...
        #expect(throws: Institute.Inventory.Effective.Error.self) {
            _ = try Institute.Inventory.Effective(public: publicConfiguration, private: discovery)
        }
    }
        ...
        throws
    {
        ...
        let effective = try Institute.Inventory.Effective(public: publicConfiguration, private: discovery)
        ...
        throws
    {
        ...
        let output = Institute.Inventory.Effective.Output(
            scope: .public,
            effective: effective,
            unmeasured: []
        )
    ...
    func `Output is byte-identical across two runs over identical inputs`() throws {
        func report() throws -> Institute.Inventory.Effective.Output {
            let effective = try Institute.Inventory.Effective(
                public: Self.publicConfiguration,
    ...
    func `Changed input changes the combined digest and the report bytes`() throws {
        let one = try Institute.Inventory.Effective(
            public: Self.publicConfiguration,
            private: Self.privateDiscovery
        )
        let two = try Institute.Inventory.Effective(
            public: Self.publicConfiguration,
            private: .init(repositories: [], exclusions: [], unmeasured: [])
        )

        let first = Institute.Inventory.Effective.Output(
            scope: .effective, effective: one, unmeasured: []
        )
        let second = Institute.Inventory.Effective.Output(
    ...
    func `Output round-trips through its own serialization`() throws {
        ...
        let decoded = try Institute.Inventory.Effective.Output(jsonString: output.canonical)
    ...
    private static var publicConfiguration: Institute.Configuration {
        .init(
            version: 1,
            scope: "swift-institute",
            swift: "6.3.3",
    ...
    private static var privateDiscovery: Institute.Inventory.Private.Discovery {
        .init(
            repositories: [
                .init(
                    id: .init(1),
...
extension Institute.Inventory.Test.Unit {
        ...
        throws
    {
        ...
        let live = try Institute.Inventory.Effective(
            public: configuration,
            private: Self.privateDiscovery
        )
        let supplied = try Institute.Inventory.Effective(
            public: configuration,
            roster: .init(
                repositories: [
                    .init(
    ...
    func `A permuted roster digests identically`() throws {
        let rows: [Institute.Repository.Key] = [
        ...
        let forward = try Institute.Inventory.Effective(
            public: Self.publicConfiguration,
            roster: .init(repositories: rows, unmeasured: []),
            policy: .institute()
        )
        let reversed = try Institute.Inventory.Effective(
            public: Self.publicConfiguration,
    ...
    func `A coordinate outside the policy's organizations is refused`() throws {
        #expect(throws: Institute.Inventory.Effective.Error.self) {
            try Institute.Inventory.Effective(
                public: Self.publicConfiguration,
                roster: .init(
